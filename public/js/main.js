...
(function(window) {
  ('use strict');
  const rowsContainer = document.querySelector('.rows--mini');
  let innerHTML = '';
  const availablePlaces = Object.values(
    JSON.parse(localStorage.getItem('availablePlaces'))
  );
  
  for (let i = 0; i < 18; i++) {
    let result = `<div class="row">`;
    for (let j = 18 * i; j < 18 * (i + 1); j++) {
      const el = availablePlaces[j];
      if (el === 'null') {
        result += `<div class="row__seat row__seat--reserved"></div>`;
        continue;
      }
      result += `<div class="row__seat tooltip" data-tooltip="${el}"></div>`;
    }
    result += '</div>';
    innerHTML += result;
  }

  rowsContainer.innerHTML = innerHTML;
  ...
  function applyRoomTransform(transform) {
    room.style.WebkitTransform = room.style.transform = transform
      ...
      : 'translate3d(0,0,' +
        ...
        'deg) translate3d(' +
        roomTransform.translateX +
        'px, ' +
        roomTransform.translateY +
        'px, ' +
        roomTransform.translateZ +
        'px)';
  }

  function applyRoomTransition(settings) {
    var settings = settings || transitionOpts;
    room.style.WebkitTransition =
      '-webkit-transform ' + settings.speed + 'ms ' + settings.easing;
    room.style.transition =
      'transform ' + settings.speed + 'ms ' + settings.easing;
  }
  ...
  function scaleRoom() {
    ...
    container.style.WebkitTransform = container.style.transform =
      'scale3d(' + factor + ',' + factor + ',1)';
  }

  function initEvents() {
    // select a seat
    var onSeatSelect = function(ev) {
    ...
    document.addEventListener('DOMContentLoaded', onSelectSeats);
  ...
  function previewSeat(seat) {
    ...
    var values = tr.split('(')[1],
      ...
      // for the last row:
      minRotY_1 = 0,
      maxRotY_1 = 20, // min and max values for y rotation
      ...
      // for the first row:
      minRotY_2 = 0,
      maxRotY_2 = 50, // min and max values for y rotation
      rotY_2 = lineEq(
        minRotY_2,
        maxRotY_2,
        initialTranslationX,
  ...
  function buyTickets() {
    const seats = Array.from(document.querySelectorAll('.row__seat--selected'));
    localStorage.setItem(
      'selectedPlaces',
      JSON.stringify(seats.map(el => el.getAttribute('data-tooltip')))
    );
    window.history.back();
  }

  var buyBtn = document.querySelector('.action--buy');
